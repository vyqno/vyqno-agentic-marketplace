...
export async function POST(request: NextRequest) {
  try {
    ...
    // Store pending transaction (best-effort — don't fail if table missing)
    try {
      ...
      console.error("[topup] DB insert failed (non-fatal):", dbErr);
    }
    ...
    console.error("[topup/create-order]", err);
    return NextResponse.json(
